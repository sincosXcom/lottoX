...
def render_prediction_page():
    ...
    # 加载实际开奖数据
    draws_df = load_sheet_data(
        "kl8_draws",
        ["issue"] + [f"n{i}" for i in range(1,21)],
        st.secrets["google"]["predict_spreadsheet_id"]
    )
    ...
    if not draws_df.empty:
        for _, row in draws_df.iterrows():
            ...
            draw_map[issue] = set(nums)
    if 'target_issue' in df_pred.columns:
        df_pred['target_issue_num'] = pd.to_numeric(df_pred['target_issue'], errors='coerce')
        df_pred = df_pred.sort_values(['model', 'feature_mode', 'temperature', 'target_issue_num']).reset_index(drop=True)
    else:
        df_pred = df_pred.sort_values(['model', 'feature_mode', 'temperature', 'timestamp']).reset_index(drop=True)
    models = sorted(df_pred['model'].unique())
    modes = sorted(df_pred['feature_mode'].unique())
    temps = sorted(df_pred['temperature'].unique())
    st.markdown("""
    <style>
    .pred-table {
        width: 100%;
    ...
    for model in models:
        for mode in modes:
            for temp in temps:
                ...
                headers = ['期号'] + [str(i) for i in range(1, max_cols+1)] + ['命中数']
                html_table = '<table class="pred-table"><thead><tr>' + ''.join(f'<th>{h}</th>' for h in headers) + '</tr></thead><tbody>'
                for _, row in hist.iterrows():
                    pred_nums = row['predicted_numbers'].split(',')
                    hit_val = row['hit_count']
                    hit_count = int(float(hit_val)) if pd.notna(hit_val) and hit_val != '' else ''
                    issue_display = row['target_issue'] if 'target_issue' in row else row['timestamp'][:10]
                    actual_set = draw_map.get(str(issue_display), set()) if issue_display else set()
                    html_table += '<tr>'
                    html_table += f'<td style="white-space: nowrap;">{issue_display}</td>'
                    for pos in range(max_cols):
                        if pos < len(pred_nums):
                            num = int(pred_nums[pos])
                            if num in actual_set:
                                html_table += f'<td><div class="number-ball red-ball">{num:02d}</div></td>'
                            else:
                                html_table += f'<td><div class="number-ball">{num:02d}</div></td>'
                        else:
                            html_table += '<td></td>'
                    html_table += f'<td style="font-weight: bold; font-size: 16px;">{hit_count}</td>'
                    html_table += '</tr>'
...
def main():
    ...
    if not st.session_state.vip_unlocked:
        ...
        if st.sidebar.button("🎱 快乐8 智能预测", use_container_width=True，primary):
    ...
    if st.session_state.get("vip_unlocked", False) and st.session_state.get("show_trend", False):
        ...
        st.session_state.show_trend = False
        st.markdown("---")
    elif st.session_state.get("vip_unlocked", False) and st.session_state.get("show_prediction", False):
        render_prediction_page()
        if st.sidebar.button("🔙 返回开奖结果"):
            st.session_state.show_prediction = False
            st.rerun()
    elif st.session_state.get("vip_unlocked", False) and st.session_state.get("show_advanced", False):
        render_advanced_analysis(st.session_state.advanced_lottery)
        if st.sidebar.button("🔙 返回开奖结果"):
            st.session_state.show_advanced = False
            st.rerun()
    else:
